...
impl CompileConfig {
    ...
    fn new(
        ...
        let mut inner = RustCompileConfig::default();
        inner.max_levels = max_levels;
        inner.coarse_step_deg = coarse_step_deg;
        inner.min_step_deg = min_step_deg;
        inner.fill_value = fill_value;
        inner.precompute_coarsest = precompute_coarsest;
...
impl MatchConfig {
    ...
    fn new(
        ...
        let mut inner = RustMatchConfig::default();
        inner.metric = metric;
        inner.rotation = rotation;
        inner.parallel = parallel;
        inner.max_image_levels = max_image_levels;
        inner.beam_width = beam_width;
        inner.per_angle_topk = per_angle_topk;
        inner.nms_radius = nms_radius;
        inner.roi_radius = roi_radius;
        inner.angle_half_range_steps = angle_half_range_steps;
        inner.min_var_i = min_var_i;
...
impl Template {
    ...
    fn compile(&self, config: Option<CompileConfig>) -> PyResult<CompiledTemplate> {
        ...
        Ok(CompiledTemplate::new(compiled))
    }
    ...
    fn compile_no_rotation(&self, max_levels: usize) -> PyResult<CompiledTemplate> {
        let mut cfg = RustCompileConfigNoRot::default();
...
///
/// The compiled template assets are stored behind an ``Arc`` so that
/// ``matcher()`` can be called multiple times with different configurations
/// without recompiling. Each call is cheap (clones the ``Arc`` pointer only).
#[pyclass]
pub struct CompiledTemplate {
    compiled: Arc<RustCompiledTemplate>,
    num_levels: usize,
}

impl CompiledTemplate {
    fn new(compiled: RustCompiledTemplate) -> Self {
...
impl CompiledTemplate {
    ...
    /// configurations is cheap (the compiled template assets are shared via
    /// ``Arc`` and are never recomputed).
    ///
    /// Args:
    ...
    #[pyo3(signature = (config = None))]
    fn matcher(&self, config: Option<MatchConfig>) -> PyResult<Matcher> {
        let cfg = config.map(|c| c.inner).unwrap_or_default();
        cfg.validate().map_err(to_py_err)?;
        let inner = RustMatcher::from_arc(Arc::clone(&self.compiled)).with_config(cfg);
        Ok(Matcher { inner })
    }

    fn __repr__(&self) -> String {
        format!("CompiledTemplate(num_levels={})", self.num_levels)
    }
}
...
fn match_template(
    ...
    // Create match config
    let mut match_config = RustMatchConfig::default();
    match_config.metric = metric_val;
    match_config.rotation = rotation_mode;
    match_config.parallel = parallel;
