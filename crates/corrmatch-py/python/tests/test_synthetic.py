...
class TestMatcherReuse:
    ...
    def test_matcher_reusable_after_first_call(self):
        ...
        result1 = matcher1.match_image(image)

        # Second matcher from the same CompiledTemplate — different config
        matcher2 = compiled.matcher(corrmatch.MatchConfig(beam_width=8))
        result2 = matcher2.match_image(image)

        # Both should find the template at the planted position
        assert abs(result1.x - 50) <= 2.0, f"matcher1 x error: {result1.x}"
        assert abs(result1.y - 50) <= 2.0, f"matcher1 y error: {result1.y}"
        assert result1.score > 0.9

        assert abs(result2.x - 50) <= 2.0, f"matcher2 x error: {result2.x}"
        assert abs(result2.y - 50) <= 2.0, f"matcher2 y error: {result2.y}"
