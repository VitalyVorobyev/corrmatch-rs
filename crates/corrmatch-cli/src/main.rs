...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let compiled = match config.match_cfg.rotation {
        RotationModeConfig::Enabled => {
            let mut compile_cfg = CompileConfig::default();
            compile_cfg.max_levels = config.compile.max_levels;
            compile_cfg.coarse_step_deg = config.compile.coarse_step_deg;
            compile_cfg.min_step_deg = config.compile.min_step_deg;
            compile_cfg.fill_value = config.compile.fill_value;
            compile_cfg.precompute_coarsest = config.compile.precompute_coarsest;
            CompiledTemplate::compile_rotated(&template, compile_cfg)?
        }
        RotationModeConfig::Disabled => {
            let mut norot_cfg = CompileConfigNoRot::default();
            norot_cfg.max_levels = config.compile.max_levels;
            CompiledTemplate::compile_unrotated(&template, norot_cfg)?
        }
    };

    let mut match_cfg = MatchConfig::default();
    match_cfg.metric = config.match_cfg.metric.into();
    match_cfg.rotation = config.match_cfg.rotation.into();
    match_cfg.parallel = config.match_cfg.parallel;
    match_cfg.max_image_levels = config.match_cfg.max_image_levels;
    match_cfg.beam_width = config.match_cfg.beam_width;
    match_cfg.per_angle_topk = config.match_cfg.per_angle_topk;
    match_cfg.nms_radius = config.match_cfg.nms_radius;
    match_cfg.roi_radius = config.match_cfg.roi_radius;
    match_cfg.angle_half_range_steps = config.match_cfg.angle_half_range_steps;
    match_cfg.min_var_i = config.match_cfg.min_var_i;
    match_cfg.min_score = config.match_cfg.min_score;
    let matcher = Matcher::new(compiled).with_config(match_cfg);
