...
/// Compiled template assets with rotation support (internal implementation detail).
pub(crate) struct CompiledTemplateRot {
...
pub(crate) struct CompiledTemplateNoRot {
    levels: Vec<OwnedImage>,
    unmasked_zncc: Vec<TemplatePlan>,
    unmasked_ssd: Vec<SsdTemplatePlan>,
}
...
/// Private enum holding the actual compiled assets.
enum CompiledTemplateKind {
...
///
/// This type is opaque — the internal representation (rotated vs. unrotated)
/// is an implementation detail. Use `Template::compile` /
/// `CompiledTemplate::compile_rotated` when rotation search is required, or
/// `CompiledTemplate::compile_unrotated` for the fast translation-only path.
pub struct CompiledTemplate {
    kind: CompiledTemplateKind,
}
...
impl CompiledTemplate {
    ...
    pub fn compile_rotated(tpl: &Template, cfg: CompileConfig) -> CorrMatchResult<Self> {
        Ok(Self {
            kind: CompiledTemplateKind::Rotated(CompiledTemplateRot::compile(tpl, cfg)?),
        })
    }
    ...
    pub fn compile_unrotated(tpl: &Template, cfg: CompileConfigNoRot) -> CorrMatchResult<Self> {
        Ok(Self {
            kind: CompiledTemplateKind::Unrotated(CompiledTemplateNoRot::compile(tpl, cfg)?),
        })
    }

    /// Returns `true` if this template was compiled with rotation support.
    pub fn is_rotated(&self) -> bool {
        matches!(self.kind, CompiledTemplateKind::Rotated(_))
    }

    /// Returns the number of pyramid levels.
    pub fn num_levels(&self) -> usize {
        match &self.kind {
            CompiledTemplateKind::Rotated(rot) => rot.num_levels(),
            CompiledTemplateKind::Unrotated(unrot) => unrot.num_levels(),
        }
    }
    ...
    pub fn level_size(&self, level: usize) -> Option<(usize, usize)> {
        match &self.kind {
            CompiledTemplateKind::Rotated(rot) => rot.level_size(level),
            CompiledTemplateKind::Unrotated(unrot) => unrot.level_size(level),
        }
    }
    ...
    pub fn angle_grid(&self, level: usize) -> Option<&AngleGrid> {
        match &self.kind {
            CompiledTemplateKind::Rotated(rot) => rot.angle_grid(level),
            CompiledTemplateKind::Unrotated(_) => None,
        }
    }
    ...
    pub fn unmasked_zncc_plan(&self, level: usize) -> CorrMatchResult<&TemplatePlan> {
        match &self.kind {
            CompiledTemplateKind::Rotated(rot) => rot.unmasked_zncc_plan(level),
            CompiledTemplateKind::Unrotated(unrot) => unrot.unmasked_zncc_plan(level),
        }
    }
    ...
    pub fn unmasked_ssd_plan(&self, level: usize) -> CorrMatchResult<&SsdTemplatePlan> {
        match &self.kind {
            CompiledTemplateKind::Rotated(rot) => rot.unmasked_ssd_plan(level),
            CompiledTemplateKind::Unrotated(unrot) => unrot.unmasked_ssd_plan(level),
        }
    }
    ...
    pub(crate) fn rotated(
        ...
        match &self.kind {
            CompiledTemplateKind::Rotated(rot) => rot.rotated(level, angle_idx),
            CompiledTemplateKind::Unrotated(_) => Err(CorrMatchError::RotationUnavailable {
