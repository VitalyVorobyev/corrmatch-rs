...
impl Matcher {
    ...
    ///
    /// Ownership of the compiled template is transferred. Use
    /// [`Matcher::from_arc`] to share a compiled template across multiple
    /// matchers without recompiling.
    ...
    /// Creates a matcher from a shared compiled template.
    ///
    /// Cloning the `Arc` is cheap; the compiled template assets are not
    /// duplicated. This allows constructing multiple matchers with different
    /// [`MatchConfig`]s from a single compiled template.
    pub fn from_arc(compiled: Arc<CompiledTemplate>) -> Self {
        Self {
            compiled,
            cfg: MatchConfig::default(),
        }
    ...
    fn match_candidates_with_cfg(
        ...
        if !self.compiled.is_rotated() && cfg.rotation == RotationMode::Enabled {
